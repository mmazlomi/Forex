...
function getRiskSettings(userId, mode) {
  const db = getDb();
  return db.prepare('SELECT * FROM risk_settings WHERE user_id = ? AND mode = ?').get(userId, mode);
}
...
function upsertRiskSettings(userId, mode, settings) {
  ...
  db.prepare(
    `INSERT INTO risk_settings (
       ...
       max_order_value, min_risk_reward_ratio, max_portfolio_exposure_percent, updated_at_utc
     ) VALUES (
       @userId, @mode, @maxRiskPerTradePercent, @maxDailyLossPercent, @maxOpenPositions,
       @maxOrderValue, @minRiskRewardRatio, @maxPortfolioExposurePercent, @updatedAtUtc
     )
