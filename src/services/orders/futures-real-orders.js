 ...
 */
async function placeRealFuturesOrder({ userId, symbol, exchange = 'kucoin', action, leverage, stopLoss, takeProfit, qty, idempotencyKey, signalId, unlockConfirmed, source = 'manual' }) {
  const id = uuidv4();
  action = String(action || '').toLowerCase();

  // 1. Live-trading gate — checked first, before any other work.
  if (!config.enableLiveTrading) {
    return persistRejected({ id, userId, symbol, action, leverage, stopLoss, takeProfit, reasonCode: 'LIVE_TRADING_DISABLED', message: 'ENABLE_LIVE_TRADING is not set to true in .env.', idempotencyKey, signalId, source });
  }
  const credentials = resolveRealCredentials(userId);
  if (!credentials.name || !credentials.apiKey || !credentials.apiSecret) {
    return persistRejected({ id, userId, symbol, action, leverage, stopLoss, takeProfit, reasonCode: 'MISSING_REAL_CREDENTIALS', message: 'Real exchange credentials are not fully configured (set them from the Real Trading tab, or via REAL_EXCHANGE_NAME/REAL_API_KEY/REAL_API_SECRET in .env).', idempotencyKey, signalId, source });
  }
  if (credentials.name.toLowerCase() !== 'kucoin') {
    return persistRejected({ id, userId, symbol, action, leverage, stopLoss, takeProfit, reasonCode: 'FUTURES_EXCHANGE_UNSUPPORTED', message: `Futures trading is only supported on KuCoin; the configured Real exchange is "${credentials.name}".`, idempotencyKey, signalId, source });
  }
  if (!unlockConfirmed) {
    return persistRejected({ id, userId, symbol, action, leverage, stopLoss, takeProfit, reasonCode: 'REAL_TRADING_NOT_UNLOCKED', message: 'The UI real-trading unlock/confirmation step was not completed for this request.', idempotencyKey, signalId, source });
  }
  ...
  if (emergencyStopRepository.isActive(MODE, userId)) {
    return persistRejected({ id, userId, symbol, action, leverage, stopLoss, takeProfit, reasonCode: 'EMERGENCY_STOP_ACTIVE', message: 'Real futures trading is halted by an active emergency stop.', idempotencyKey, signalId, source });
  }
  ...
  try {
    ...
    return persistRejected({ id, userId, symbol, action, leverage, stopLoss, takeProfit, price, reasonCode: 'EXCHANGE_UNAVAILABLE', message: `Could not fetch live futures balance: ${err.message}`, idempotencyKey, signalId, source });
  }
  ...
  // Unlike Bybit (separate setLeverage/setMarginMode calls before createOrder), KuCoin sets
  // leverage and margin mode as part of the order itself — confirmed by reading ccxt's
  ...
  // marginMode) — calling it here would be actively wrong for the isolated-margin default this
  // app uses, not just redundant.
  ...
  try {
    ...
    return persistRejected({ id, userId, symbol, action, leverage, stopLoss, takeProfit, price, reasonCode: 'EXCHANGE_ORDER_FAILED', message: err.message, idempotencyKey, signalId, source });
  }
...
async function closeRealFuturesPosition({ id, userId, symbol, exchange, price, idempotencyKey, signalId, source }) {
  ...
  if (!openPosition) {
    return persistRejected({ id, userId, symbol, action: 'close', price, reasonCode: 'NO_OPEN_POSITION_TO_CLOSE', message: `No open Real futures position for ${symbol}.`, idempotencyKey, signalId, source });
  }
  ...
  try {
    ...
    return persistRejected({ id, userId, symbol, action: 'close', price, reasonCode: 'EXCHANGE_ORDER_FAILED', message: err.message, idempotencyKey, signalId, source });
  }
